...
def _report(start: date, end: date, ids: list[int], include_unapproved: bool, db: Session) -> FinancialReportView:
    ...
    for branch in branches:
        ...
        while cursor <= end:
            ...
            daily_rows.append(ReportDailyRow(business_date=cursor, branch_id=branch.id, branch_name=branch.name, revenue=revenue, customer_count=customers, revenue_per_customer=_per_customer(revenue, customers), allocated_expense=expense, net_profit=revenue - expense, entry_status=status))
            cursor += timedelta(days=1)
        branch_expenses = sum(allocation.values(), Decimal(0))
        summaries.append(ReportBranchSummary(branch_id=branch.id, branch_name=branch.name, revenue=branch_revenue, customer_count=branch_customers, revenue_per_customer=_per_customer(branch_revenue, branch_customers), expenses=branch_expenses, net_profit=branch_revenue - branch_expenses, approved_entries=approved, missing_days=missing))
        company_revenue += branch_revenue
        company_expenses += branch_expenses
        company_customers += branch_customers
    return FinancialReportView(date_from=start, date_to=end, generated_at=datetime.now(timezone.utc).isoformat(), company_revenue=company_revenue, company_customer_count=company_customers, company_revenue_per_customer=_per_customer(company_revenue, company_customers), company_expenses=company_expenses, company_net_profit=company_revenue - company_expenses, branches=summaries, daily_rows=daily_rows)
