...
def _report(start: date, end: date, ids: list[int], include_unapproved: bool, db: Session) -> FinancialReportView:
    ...
    entries = list(db.scalars(query)) if ids else []
    entry_map = {(x.branch_id, x.business_date): x for x in entries}
    ...
    for branch in branches:
        ...
        cursor = start
        while cursor <= end:
            entry = entry_map.get((branch.id, cursor))
            ...
            daily_rows.append(ReportDailyRow(business_date=cursor, branch_id=branch.id, branch_name=branch.name, revenue=gross, company_percentage=company_pct, hotel_percentage=hotel_pct, company_share=company_share, hotel_share=hotel_share, customer_count=customers, allocated_expense=expense, net_profit=company_share-expense, entry_status=status))
            gross_sum += gross; company_sum += company_share; hotel_sum += hotel_share; customer_sum += customers
            approved += int(bool(entry and entry.status == "approved")); missing += int(entry is None)
            cursor += timedelta(days=1)
        expenses = sum(allocation.values(), Decimal(0))
        summaries.append(ReportBranchSummary(branch_id=branch.id, branch_name=branch.name, revenue=gross_sum, company_percentage=company_pct, hotel_percentage=hotel_pct, company_share=company_sum, hotel_share=hotel_sum, customer_count=customer_sum, expenses=expenses, net_profit=company_sum-expenses, approved_entries=approved, missing_days=missing))
        gross_total += gross_sum; company_total += company_sum; hotel_total += hotel_sum; expense_total += expenses; customer_total += customer_sum
    return FinancialReportView(date_from=start, date_to=end, generated_at=datetime.now(timezone.utc).isoformat(), company_revenue=gross_total, company_share=company_total, hotel_share=hotel_total, company_customer_count=customer_total, company_expenses=expense_total, company_net_profit=company_total-expense_total, branches=summaries, daily_rows=daily_rows)
...
def _name(report, extension): return f"financial_report_{report.date_from}_{report.date_to}.{extension}"
def _money(value): return f"{value:,.0f} IQD"
def _add(headers, values, enabled, header, value):
    if enabled: headers.append(header); values.append(value)


@router.get("/export/excel")
def excel(date_from: date, date_to: date, branch_ids: list[int] | None = Query(default=None), include_unapproved: bool = False, show_revenue: bool = True, show_expenses: bool = True, show_profit: bool = True, show_customers: bool = True, show_revenue_sharing: bool = True, show_branch_summary: bool = True, show_daily_details: bool = True, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    wb = Workbook(); summary = wb.active; summary.title = "Summary"; dark, red = "073C46", "D62828"
    ...
    headers, values = [], []
    _add(headers, values, show_revenue, "Gross Revenue", float(report.company_revenue))
    if show_revenue_sharing: headers += ["Company Share", "Hotel Share"]; values += [float(report.company_share), float(report.hotel_share)]
    _add(headers, values, show_customers, "Customers", report.company_customer_count)
    _add(headers, values, show_expenses, "Expenses", float(report.company_expenses))
    _add(headers, values, show_profit, "Company Net Profit", float(report.company_net_profit))
    summary.append(headers); summary.append(values); summary.append([])
    if show_branch_summary:
        ...
        if show_customers: headers.append("Customers")
        if show_expenses: headers.append("Expenses")
        if show_profit: headers.append("Company Net Profit")
        headers += ["Approved Days", "Missing Days"]; summary.append(headers)
        ...
        for item in report.branches:
            ...
            if show_customers: row.append(item.customer_count)
            if show_expenses: row.append(float(item.expenses))
            if show_profit: row.append(float(item.net_profit))
            row += [item.approved_entries,item.missing_days]; summary.append(row)
    ...
    if show_revenue_sharing:
        sharing=wb.create_sheet("Revenue Sharing"); headers=["Branch"]
        if show_revenue: headers.append("Gross Revenue")
    ...
    if show_daily_details:
        details=wb.create_sheet("Daily Details"); headers=["Date","Branch"]
        if show_revenue: headers.append("Gross Revenue")
        if show_revenue_sharing: headers += ["Company %","Hotel %","Company Share","Hotel Share"]
        if show_customers: headers.append("Customers")
        if show_expenses: headers.append("Fixed Daily Expense")
        if show_profit: headers.append("Company Net Profit")
        headers.append("Status"); details.append(headers)
        ...
        for item in report.daily_rows:
            ...
            if show_profit: row.append(float(item.net_profit))
            row.append(item.entry_status); details.append(row)
            if show_profit and item.net_profit < 0:
...
def pdf(date_from: date, date_to: date, branch_ids: list[int] | None = Query(default=None), include_unapproved: bool = False, show_revenue: bool = True, show_expenses: bool = True, show_profit: bool = True, show_customers: bool = True, show_revenue_sharing: bool = True, show_branch_summary: bool = True, show_daily_details: bool = True, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require(current_user,"reports.export_pdf"); report=_report(date_from,date_to,_branch_ids(current_user,branch_ids,db),include_unapproved,db)
    ...
    headers,values=[],[]; _add(headers,values,show_revenue,"Gross Revenue",_money(report.company_revenue))
    if show_revenue_sharing: headers += ["Company Share","Hotel Share"]; values += [_money(report.company_share),_money(report.hotel_share)]
    _add(headers,values,show_customers,"Customers",f"{report.company_customer_count:,}"); _add(headers,values,show_expenses,"Expenses",_money(report.company_expenses)); _add(headers,values,show_profit,"Company Net Profit",_money(report.company_net_profit))
    ...
    if show_profit and report.company_net_profit < 0:
        idx=headers.index("Company Net Profit"); commands += [("BACKGROUND",(idx,1),(idx,1),red),("TEXTCOLOR",(idx,1),(idx,1),colors.white)]
    ...
    if show_branch_summary:
        ...
        headers += ["Approved","Missing"]; rows=[headers]
        for item in report.branches:
            row=[item.branch_name]
            ...
            if show_customers: row.append(f"{item.customer_count:,}")
            if show_expenses: row.append(_money(item.expenses))
            if show_profit: row.append(_money(item.net_profit))
            row += [item.approved_entries,item.missing_days]; rows.append(row)
        table=Table(rows,repeatRows=1); commands=[("BACKGROUND",(0,0),(-1,0),dark),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("GRID",(0,0),(-1,-1),.3,colors.grey),("FONTSIZE",(0,0),(-1,-1),6.8),("ALIGN",(1,1),(-1,-1),"RIGHT")]
        if show_profit:
    ...
    if show_daily_details:
        ...
        if show_profit: headers.append("Company Net")
        headers.append("Status"); rows=[headers]
        for item in report.daily_rows:
            ...
            if show_profit: row.append(_money(item.net_profit))
            row.append(item.entry_status); rows.append(row)
        table=Table(rows,repeatRows=1); commands=[("BACKGROUND",(0,0),(-1,0),dark),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("GRID",(0,0),(-1,-1),.25,colors.grey),("FONTSIZE",(0,0),(-1,-1),6),("ALIGN",(2,1),(-2,-1),"RIGHT")]
        if show_profit:
            profit_col=headers.index("Company Net")
