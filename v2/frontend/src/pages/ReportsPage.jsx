...
export default function ReportsPage({user}){
  ...
  return <>
    ...
    {report&&<>
      ...
      {content.branchSummary&&<section className="reportPanel"><div className="panelHeading"><span className="eyebrow">Center Comparison</span><h3>Branch Summary</h3></div><div className="responsiveTable"><table><thead><tr><th>Center</th>{content.revenue&&<th>Gross Revenue</th>}{content.revenueSharing&&<><th>Company %</th><th>Hotel %</th><th>Company Share</th><th>Hotel Share</th></>}<th>Customers</th>{content.expenses&&<th>Expenses</th>}{content.profit&&<th>Company Net Profit</th>}<th>Approved</th><th>Missing</th></tr></thead><tbody>{report.branches.map(item=><tr key={item.branch_id}><td><strong>{item.branch_name}</strong></td>{content.revenue&&<td>{formatIQD(item.revenue)}</td>}{content.revenueSharing&&<><td>{formatPercent(item.company_percentage)}</td><td>{formatPercent(item.hotel_percentage)}</td><td>{formatIQD(item.company_share)}</td><td>{formatIQD(item.hotel_share)}</td></>}<td>{formatCount(item.customer_count)}</td>{content.expenses&&<td>{formatIQD(item.expenses)}</td>}{content.profit&&<td className={Number(item.net_profit)<0?'negativeProfitCell':''}>{formatIQD(item.net_profit)}</td>}<td>{item.approved_entries}</td><td>{item.missing_days}</td></tr>)}</tbody></table></div></section>}
      {content.dailyDetails&&<section className="reportPanel"><div className="panelHeading"><span className="eyebrow">Detailed Ledger</span><h3>Daily Financial Breakdown</h3></div><div className="responsiveTable reportDetailsTable"><table><thead><tr><th>Date</th><th>Center</th>{content.revenue&&<th>Gross Revenue</th>}{content.revenueSharing&&<><th>Company %</th><th>Hotel %</th><th>Company Share</th><th>Hotel Share</th></>}<th>Customers</th>{content.expenses&&<th>Fixed Daily Expense</th>}{content.profit&&<th>Company Net Profit</th>}<th>Status</th></tr></thead><tbody>{report.daily_rows.map(item=><tr key={`${item.branch_id}-${item.business_date}`}><td>{item.business_date}</td><td>{item.branch_name}</td>{content.revenue&&<td>{formatIQD(item.revenue)}</td>}{content.revenueSharing&&<><td>{formatPercent(item.company_percentage)}</td><td>{formatPercent(item.hotel_percentage)}</td><td>{formatIQD(item.company_share)}</td><td>{formatIQD(item.hotel_share)}</td></>}<td>{formatCount(item.customer_count)}</td>{content.expenses&&<td>{formatIQD(item.allocated_expense)}</td>}{content.profit&&<td className={Number(item.net_profit)<0?'negativeProfitCell':''}>{formatIQD(item.net_profit)}</td>}<td><span className={`statusBadge status-${item.entry_status}`}>{item.entry_status}</span></td></tr>)}</tbody></table></div></section>}
    </>}
  </>
}
