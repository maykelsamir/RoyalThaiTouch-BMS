...
const defaultContent={revenue:true,expenses:true,profit:true,revenueSharing:true,branchSummary:true,dailyDetails:true}
...
export default function ReportsPage({user}){
  ...
  const selectedMetricCount=Number(content.revenue)+Number(content.expenses)+Number(content.profit)+Number(content.revenueSharing)

  function queryString(){const params=new URLSearchParams({date_from:filters.dateFrom,date_to:filters.dateTo,include_unapproved:String(filters.includeUnapproved),show_revenue:String(content.revenue),show_expenses:String(content.expenses),show_profit:String(content.profit),show_revenue_sharing:String(content.revenueSharing),show_branch_summary:String(content.branchSummary),show_daily_details:String(content.dailyDetails)});filters.branchIds.forEach(id=>params.append('branch_ids',id));return params.toString()}
  ...
  return <>
    ...
    {report&&<>
      <section className="reportMetrics"><article><span>Total Customers</span><strong>{formatCount(report.company_customer_count)}</strong></article><article><span>Revenue / Customer</span><strong>{formatIQD(report.company_revenue_per_customer)}</strong></article>{content.revenue&&<article><span>Gross Revenue</span><strong>{formatIQD(report.company_revenue)}</strong></article>}{content.revenueSharing&&<><article><span>Company Share</span><strong>{formatIQD(report.company_share)}</strong></article><article><span>Hotel Share</span><strong>{formatIQD(report.hotel_share)}</strong></article></>}{content.expenses&&<article><span>Fixed Expenses</span><strong>{formatIQD(report.company_expenses)}</strong></article>}{content.profit&&<article className={`profitMetric ${Number(report.company_net_profit)<0?'negativeProfit':''}`}><span>Company Net Profit</span><strong>{formatIQD(report.company_net_profit)}</strong></article>}</section>
      {content.branchSummary&&<section className="reportPanel"><div className="panelHeading"><span className="eyebrow">Center Comparison</span><h3>Branch Summary</h3></div><div className="responsiveTable"><table><thead><tr><th>Center</th>{content.revenue&&<th>Gross Revenue</th>}{content.revenueSharing&&<><th>Company %</th><th>Hotel %</th><th>Company Share</th><th>Hotel Share</th></>}<th>Customers</th><th>Revenue / Customer</th>{content.expenses&&<th>Expenses</th>}{content.profit&&<th>Company Net Profit</th>}<th>Approved</th><th>Missing</th></tr></thead><tbody>{report.branches.map(item=><tr key={item.branch_id}><td><strong>{item.branch_name}</strong></td>{content.revenue&&<td>{formatIQD(item.revenue)}</td>}{content.revenueSharing&&<><td>{formatPercent(item.company_percentage)}</td><td>{formatPercent(item.hotel_percentage)}</td><td>{formatIQD(item.company_share)}</td><td>{formatIQD(item.hotel_share)}</td></>}<td>{formatCount(item.customer_count)}</td><td>{formatIQD(item.revenue_per_customer)}</td>{content.expenses&&<td>{formatIQD(item.expenses)}</td>}{content.profit&&<td className={Number(item.net_profit)<0?'negativeProfitCell':''}>{formatIQD(item.net_profit)}</td>}<td>{item.approved_entries}</td><td>{item.missing_days}</td></tr>)}</tbody></table></div></section>}
      {content.dailyDetails&&<section className="reportPanel"><div className="panelHeading"><span className="eyebrow">Detailed Ledger</span><h3>Daily Financial Breakdown</h3></div><div className="responsiveTable reportDetailsTable"><table><thead><tr><th>Date</th><th>Center</th>{content.revenue&&<th>Gross Revenue</th>}{content.revenueSharing&&<><th>Company %</th><th>Hotel %</th><th>Company Share</th><th>Hotel Share</th></>}<th>Customers</th><th>Revenue / Customer</th>{content.expenses&&<th>Fixed Daily Expense</th>}{content.profit&&<th>Company Net Profit</th>}<th>Status</th></tr></thead><tbody>{report.daily_rows.map(item=><tr key={`${item.branch_id}-${item.business_date}`}><td>{item.business_date}</td><td>{item.branch_name}</td>{content.revenue&&<td>{formatIQD(item.revenue)}</td>}{content.revenueSharing&&<><td>{formatPercent(item.company_percentage)}</td><td>{formatPercent(item.hotel_percentage)}</td><td>{formatIQD(item.company_share)}</td><td>{formatIQD(item.hotel_share)}</td></>}<td>{formatCount(item.customer_count)}</td><td>{formatIQD(item.revenue_per_customer)}</td>{content.expenses&&<td>{formatIQD(item.allocated_expense)}</td>}{content.profit&&<td className={Number(item.net_profit)<0?'negativeProfitCell':''}>{formatIQD(item.net_profit)}</td>}<td><span className={`statusBadge status-${item.entry_status}`}>{item.entry_status}</span></td></tr>)}</tbody></table></div></section>}
    </>}
  </>
}
