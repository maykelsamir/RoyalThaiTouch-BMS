...
const emptyForm = { name:'', code:'', country:'Iraq', city:'', address:'', phone:'', email:'', whatsapp:'', website:'', facebook:'', instagram:'', tiktok:'', youtube:'', telegram:'', snapchat:'', google_maps:'', manager_name:'', opening_date:'', company_revenue_percentage:100, hotel_revenue_percentage:0, logo:'', cover_image:'', notes:'', active:true }
...
export default function BranchesPage({ user, onOpenProfile }) {
  ...
  function setCompanyShare(value){const company=Math.max(0,Math.min(100,Number(value||0)));setForm({...form,company_revenue_percentage:company,hotel_revenue_percentage:Number((100-company).toFixed(2))})}
  function setHotelShare(value){const hotel=Math.max(0,Math.min(100,Number(value||0)));setForm({...form,hotel_revenue_percentage:hotel,company_revenue_percentage:Number((100-hotel).toFixed(2))})}
  ...
  async function save(e){
    ...
    try{
      ...
      const payload={
        name:String(form.name||'').trim(),code:String(form.code||'').trim(),country:String(form.country||'').trim(),city:String(form.city||'').trim(),address:String(form.address||'').trim(),phone:String(form.phone||'').trim(),email:String(form.email||'').trim(),whatsapp:String(form.whatsapp||'').trim(),website:String(form.website||'').trim(),facebook:String(form.facebook||'').trim(),instagram:String(form.instagram||'').trim(),tiktok:String(form.tiktok||'').trim(),youtube:String(form.youtube||'').trim(),telegram:String(form.telegram||'').trim(),snapchat:String(form.snapchat||'').trim(),google_maps:String(form.google_maps||'').trim(),manager_name:String(form.manager_name||'').trim(),opening_date:form.opening_date||null,company_revenue_percentage:Number(form.company_revenue_percentage),hotel_revenue_percentage:Number(form.hotel_revenue_percentage),logo:form.logo||'',cover_image:form.cover_image||'',notes:String(form.notes||'').trim(),active:Boolean(form.active),
      }
  ...
  return <>
    ...
    {showForm&&<div className="modalBackdrop"><form className="branchModal" onSubmit={save}><div className="modalHeader"><div><span className="eyebrow">{editing?'Update Center':'New Center'}</span><h3>{editing?editing.name:'Create Branch'}</h3></div><button type="button" disabled={busy||!!imageBusy} onClick={closeForm}>×</button></div>
      <div className="branchFormGrid">
        ...
        <div className="branchSocialSection wide"><div><span className="eyebrow">Revenue Sharing</span><h4>Company and hotel share of gross revenue</h4><small>Enter either percentage. The other percentage is calculated automatically, and both must total 100%.</small></div><div className="branchSocialGrid"><label>Company percentage<input type="number" min="0" max="100" step="0.01" value={form.company_revenue_percentage} onChange={e=>setCompanyShare(e.target.value)}/></label><label>Hotel percentage<input type="number" min="0" max="100" step="0.01" value={form.hotel_revenue_percentage} onChange={e=>setHotelShare(e.target.value)}/></label></div></div>
