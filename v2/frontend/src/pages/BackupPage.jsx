...
export default function BackupPage() {
  ...
  return <>
    <div className="pageTitleRow">
      <div>
        <span className="eyebrow">Database Protection</span>
        <h2>Backup System</h2>
        <p>Create, download, restore, and remove protected PostgreSQL backups.</p>
      </div>
      <button className="primaryButton" onClick={createBackup} disabled={Boolean(busy)}>
        {busy === 'create' ? 'Creating backup…' : 'Create New Backup'}
      </button>
    </div>
    ...
    <section className="companyCard" style={{ display: 'block' }}>
      <div className="pageTitleRow">
        <div><span className="eyebrow">Backup History</span><h3>Stored Backups</h3></div>
        <button className="secondaryButton" onClick={load} disabled={Boolean(busy)}>Refresh</button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {items.map((item) => <tr key={item.filename} style={{ borderTop: '1px solid rgba(128,128,128,.25)' }}>
              <td style={{ padding: 12 }}><strong>{item.filename}</strong></td>
              <td style={{ padding: 12 }}>{new Date(item.created_at).toLocaleString()}</td>
              <td style={{ padding: 12 }}>{formatSize(item.size)}</td>
              <td style={{ padding: 12, textAlign: 'right', whiteSpace: 'nowrap' }}>
                <button className="secondaryButton" onClick={() => apiDownload(`/backup/${encodeURIComponent(item.filename)}/download`, item.filename)} disabled={Boolean(busy)}>Download</button>{' '}
