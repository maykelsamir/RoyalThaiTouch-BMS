...
function socialUrl(type,value){
  ...
  if(type==='website'||type==='google_maps')return `https://${raw}`
  const username=raw.replace(/^@/,'').replace(/^\/+|\/+$/g,'')
  const bases={facebook:'https://facebook.com/',instagram:'https://instagram.com/',tiktok:'https://tiktok.com/@',youtube:'https://youtube.com/@',telegram:'https://t.me/',snapchat:'https://snapchat.com/add/'}
  return `${bases[type]||'https://'}${username}`
}
function SocialLink({type,label,value}){const url=socialUrl(type,value);if(!url)return null;return <a className="branchSocialLink" href={url} target="_blank" rel="noopener noreferrer" title={`Open ${label} in a new tab`} onDoubleClick={(event)=>{event.preventDefault();window.open(url,'_blank','noopener,noreferrer')}}><span>{label}</span><strong>{url}</strong><small>Open in new tab ↗</small></a>}
...
export default function BranchProfilePage({ branchId, onBack, onEdit }){
  ...
  async function load(){setLoading(true);setError('');try{setData(await api(`/branches/${branchId}/profile`))}catch(e){setError(e.message)}finally{setLoading(false)}}
  ...
  const socialLinks=[['whatsapp','WhatsApp'],['website','Website'],['facebook','Facebook'],['instagram','Instagram'],['tiktok','TikTok'],['youtube','YouTube'],['telegram','Telegram'],['snapchat','Snapchat'],['google_maps','Google Maps']].filter(([key])=>branch[key])
  ...
  return <section className="branchProfilePage">
    ...
    <section className="profileMetrics"><article><span>Total Revenue</span><strong>{money(summary.total_revenue)}</strong></article><article><span>Total Expenses</span><strong>{money(summary.total_expenses)}</strong></article><article className="profit"><span>Net Profit</span><strong>{money(summary.net_profit)}</strong></article><article><span>Assigned Users</span><strong>{summary.users}</strong></article><article><span>Pending Approvals</span><strong>{summary.pending_approvals}</strong></article></section>

    <nav className="profileTabs">{['overview','financial','users','performance','documents','audit'].map(item=><button key={item} className={tab===item?'active':''} onClick={()=>setTab(item)}>{item==='audit'?'Audit History':item[0].toUpperCase()+item.slice(1)}</button>)}</nav>

    {tab==='overview'&&<><div className="profileTwoColumn">
      <article className="profilePanel"><h3>Branch Information</h3><dl className="profileDetails"><div><dt>Manager</dt><dd>{branch.manager_name||'Not assigned'}</dd></div><div><dt>Opening Date</dt><dd>{branch.opening_date||'—'}</dd></div><div><dt>Phone</dt><dd>{branch.phone||'—'}</dd></div><div><dt>Email</dt><dd>{branch.email||'—'}</dd></div><div><dt>Address</dt><dd>{branch.address||'—'}</dd></div><div><dt>Created</dt><dd>{dateTime(branch.created_at)}</dd></div><div><dt>Last Updated</dt><dd>{dateTime(branch.updated_at)}</dd></div></dl></article>
    ...
    </div>{socialLinks.length>0&&<article className="profilePanel branchSocialPanel"><div className="panelHeading"><div><h3>Social Media & Public Links</h3><p>Click or double-click any link to open it in a new browser tab.</p></div><span>{socialLinks.length} links</span></div><div className="branchSocialLinks">{socialLinks.map(([key,label])=><SocialLink key={key} type={key} label={label} value={branch[key]}/>)}</div></article>}</>}
