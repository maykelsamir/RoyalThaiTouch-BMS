...
s=s.replace("settings:'Settings'", "settings:'Settings',systemHealth:'System Health'");
...
if(!s.includes("'System Health',()=>")){
  const logout='<div className="logoutActions">';
  const healthNav="{nav('systemHealth',<Settings size={18}/>, 'System Health',()=>{setPage('systemHealth');setTimeout(loadSystemHealth,100)},['Admin','Manager'].includes(current?.role))}";
...
const sw='public/sw.js';if(fs.existsSync(sw)){let w=fs.readFileSync(sw,'utf8');w=w.replace(/const CACHE_NAME = '[^']+';/,"const CACHE_NAME = 'rtt-erp-v7-system-health';");fs.writeFileSync(sw,w)}
