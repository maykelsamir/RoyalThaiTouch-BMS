...
if(!s.includes('const canViewSystemHealth=')){
  ...
  const helper="const canViewSystemHealth=()=>{const role=String(current?.role||'').trim().toLowerCase();const permissions=(current?.permissions||[]).map(x=>String(x).toLowerCase());return ['admin','manager','accountant'].includes(role)||permissions.includes('systemhealth')||permissions.includes('system_health')||permissions.includes('users')||permissions.includes('manage_users')};";
...
.systemHealthTopBtn{border:1px solid rgba(212,175,55,.4)!important;background:rgba(212,175,55,.12)!important;color:#f4d66f!important}
...
@media(max-width:768px){.systemHealthTopBtn{display:flex!important}.healthGrid{grid-template-columns:1fr 1fr!important}.healthGrid>div{padding:14px}.healthGrid strong{font-size:15px}.healthCards{grid-template-columns:1fr 1fr!important}}
@media(max-width:420px){.healthGrid{grid-template-columns:1fr!important}.systemHealthTopBtn span{display:none}}
`;
...
if(fs.existsSync(sw)){
  ...
  w=w.replace(/const CACHE_NAME = '[^']+';/,"const CACHE_NAME = 'rtt-erp-v9-system-health-forced';");
