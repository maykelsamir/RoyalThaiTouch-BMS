...
function formatIQD(value) { return 'IQD ' + Number(value || 0).toLocaleString('en-US'); }
function todayISO() { return new Date().toISOString().slice(0, 10); }
function firstDayOfMonthISO() { const d = new Date(); return new Date(d.getFullYear(), d.getMonth(), 1).toISOString().slice(0, 10); }

function TrendChart({ data }) {
  const max = Math.max(...(data || []).map(x => Math.max(x.revenue || 0, x.expenses || 0, x.profit || 0)), 1);
  return <div className="chartWrap">{(data || []).map((x, i) => <div className="chartDay" key={x.date || i} title={`${x.date}\nRevenue: ${formatIQD(x.revenue)}\nProfit: ${formatIQD(x.profit)}`}><div className="bar revenueBar" style={{height: `${Math.max(4, (x.revenue / max) * 100)}%`}}/><div className="bar expenseBar" style={{height: `${Math.max(4, (x.expenses / max) * 100)}%`}}/><span>{String(x.date || '').slice(8,10)}</span></div>)}</div>;
}
...
function App() {
  ...
  async function loadData(dateValue = businessDate) {
    ...
    if (!entry.branch_id && branchList.length) { const branchId = String(branchList[0].id); setEntry(prev => ({ ...prev, branch_id: branchId })); setTimeout(() => loadEntry(branchId, entry.business_date), 100); }
  }

  async function loadReport() { const data = await apiFetch(`/reports/summary?date_from=${reportRange.from}&date_to=${reportRange.to}`); setReport(data); }
  ...
  {page === 'branches' && (<section className="panel"><h3>Branches</h3><table><thead><tr><th>ID</th><th>Branch</th><th>Address</th><th>Status</th></tr></thead><tbody>{branches.map(b => <tr key={b.id}><td>{b.id}</td><td>{b.name}</td><td>{b.address}</td><td>Active</td></tr>)}</tbody></table></section>)}{page === 'settings' && (<section className="panel"><h3>Settings</h3><p>Company: Royal Thai Touch</p><p>Currency: IQD</p><p>Expense default category: Other Expenses</p></section>)}</main></div>);
}
