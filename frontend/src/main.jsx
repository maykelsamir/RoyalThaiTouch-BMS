...
function App() {
  ...
  const [reportOptions, setReportOptions] = useState({ branch_id: '', include_expenses: true });
  ...
  const reportQuery = () => {
    const params = new URLSearchParams({ date_from: reportRange.from, date_to: reportRange.to, include_expenses: String(reportOptions.include_expenses) });
    if (reportOptions.branch_id) params.set('branch_id', reportOptions.branch_id);
...
{page==='settings'&&<section className="panel"><h3>Settings</h3><p>Company: Royal Thai Touch</p><p>Currency: IQD</p><p>Report options: center filter and include/exclude expenses.</p></section>}
</main></div>;
}
