...
class Employee(Base):
    ...
    salary_currency = Column(String(8), default="IQD")
    profile_image = Column(Text, nullable=True)
    passport_image = Column(Text, nullable=True)
    residence_front_image = Column(Text, nullable=True)
    residence_back_image = Column(Text, nullable=True)
...
class AppUser(Base):
    ...
    role = Column(String(80), default="Staff")
...
class SocialLink(Base):
    ...
    __table_args__ = (UniqueConstraint("branch_id", name="uq_social_branch"),)
    id = Column(Integer, primary_key=True)
    branch_id = Column(Integer, ForeignKey("branches.id"), nullable=False)
...
class MonthlyFixedExpense(Base):
    ...
    __table_args__ = (UniqueConstraint("branch_id", "month", name="uq_fixed_expense_branch_month"),)
    id = Column(Integer, primary_key=True)
    branch_id = Column(Integer, ForeignKey("branches.id"), nullable=False)
...
class PendingDailyEntry(Base):
    __tablename__ = "pending_daily_entries"
    id = Column(Integer, primary_key=True)
    branch_id = Column(Integer, ForeignKey("branches.id"), nullable=False)
    business_date = Column(Date, nullable=False)
...
class EmployeeInput(BaseModel):
    ...
    salary: float = 0
    salary_currency: str = "IQD"
    profile_image: Optional[str] = None
    passport_image: Optional[str] = None
    residence_front_image: Optional[str] = None
    residence_back_image: Optional[str] = None
    active: bool = True
...
class MonthlyFixedExpenseInput(BaseModel):
    branch_id: int
    month: str
    amount: float = 0

class PendingEntryInput(BaseModel):
    branch_id: int
    business_date: date
    revenue: float = 0
    notes: Optional[str] = None
...
def report_data(db: Session, date_from: date, date_to: date, branch_id: Optional[int] = None, report_mode: str = "full"):
    ...
    while current <= date_to:
        ...
        for row in rows:
            key = row["branch"]
            branch_totals.setdefault(key, {"branch": key, "revenue": 0, "expenses": 0, "net_profit": 0})
...
def employee_payload(e: Employee, branch_name: str = ""):
    return {"id": e.id, "branch_id": str(e.branch_id), "branch": branch_name, "name": e.name, "role": e.role or "", "phone": e.phone or "", "salary": float(e.salary or 0), "salaryCurrency": e.salary_currency or "IQD", "profileImage": e.profile_image or "", "passportImage": e.passport_image or "", "residenceFrontImage": e.residence_front_image or "", "residenceBackImage": e.residence_back_image or "", "active": bool(e.active)}
...
@app.get("/")
def root():
    return {"application": "Royal Thai Touch ERP", "status": "running", "version": "1.1.0"}

@app.get("/health")
def health():
    return {"status": "healthy", "app": "Royal Thai Touch ERP", "version": "1.1.0"}
...
def dashboard_trend(days: int = 30, db: Session = Depends(get_db)):
    ...
    while current <= end:
        ...
        revenue = sum(x["revenue"] for x in summary); expenses = sum(x["expenses"] for x in summary)
...
def save_daily_entry(body: DailyEntryInput, db: Session = Depends(get_db)):
    if body.revenue < 0 or any(item.amount < 0 for item in body.expenses):
    ...
    if revenue and revenue.closed: raise HTTPException(status_code=403, detail="Day is closed")
    if not revenue:
        revenue = DailyRevenue(branch_id=body.branch_id, business_date=body.business_date, amount=body.revenue, notes=body.notes)
        db.add(revenue); action = "CREATE_DAILY_ENTRY"
    else:
        revenue.amount = body.revenue; revenue.notes = body.notes; action = "UPDATE_DAILY_ENTRY"
...
def close_day(branch_id: int, business_date: date, db: Session = Depends(get_db)):
    revenue = db.query(DailyRevenue).filter(DailyRevenue.branch_id == branch_id, DailyRevenue.business_date == business_date).first()
    if not revenue: raise HTTPException(status_code=404, detail="No revenue entry found")
    revenue.closed = True
...
def get_employees(db: Session = Depends(get_db)):
    ...
    return [employee_payload(e, branches.get(e.branch_id, "-")) for e in db.query(Employee).filter(Employee.active == True).order_by(Employee.id.desc()).all()]

@app.post("/employees")
def create_employee(body: EmployeeInput, db: Session = Depends(get_db)):
    e = Employee(branch_id=body.branch_id, name=body.name, role=body.role, phone=body.phone, salary=body.salary, salary_currency=body.salary_currency, profile_image=body.profile_image, passport_image=body.passport_image, residence_front_image=body.residence_front_image, residence_back_image=body.residence_back_image, active=body.active)
    db.add(e); create_audit(db, "CREATE", "Employee", body.name); db.commit(); db.refresh(e)
    branch = db.query(Branch).filter(Branch.id == e.branch_id).first()
    return employee_payload(e, branch.name if branch else "-")
...
def delete_employee(employee_id: int, db: Session = Depends(get_db)):
    ...
    e.active = False; create_audit(db, "DELETE", "Employee", e.name); db.commit(); return {"status": "deleted"}

@app.get("/users")
def get_users(db: Session = Depends(get_db)):
    return db.query(AppUser).order_by(AppUser.id.desc()).all()
...
def create_user(body: UserInput, db: Session = Depends(get_db)):
    ...
    u = AppUser(username=body.username, secret=body.secret, role=body.role, permissions=body.permissions, allowed_branches=body.allowed_branches, active=body.active)
    db.add(u); create_audit(db, "CREATE", "User", body.username); db.commit(); db.refresh(u); return u

@app.delete("/users/{user_id}")
def delete_user(user_id: int, db: Session = Depends(get_db)):
    u = db.query(AppUser).filter(AppUser.id == user_id).first()
    if not u: raise HTTPException(status_code=404, detail="User not found")
    db.delete(u); create_audit(db, "DELETE", "User", u.username); db.commit(); return {"status": "deleted"}

@app.get("/social-links")
def get_social_links(db: Session = Depends(get_db)):
    return db.query(SocialLink).order_by(SocialLink.branch_id).all()

@app.post("/social-links")
def upsert_social_links(body: SocialLinkInput, db: Session = Depends(get_db)):
    s = db.query(SocialLink).filter(SocialLink.branch_id == body.branch_id).first()
    if not s:
        s = SocialLink(branch_id=body.branch_id); db.add(s)
    for field in ["facebook", "instagram", "tiktok", "whatsapp", "website", "notes"]:
        setattr(s, field, getattr(body, field))
    s.updated_at = datetime.utcnow(); create_audit(db, "UPSERT", "SocialLink", f"branch_id={body.branch_id}"); db.commit(); db.refresh(s); return s

@app.get("/monthly-fixed-expenses")
def get_monthly_fixed_expenses(month: Optional[str] = None, db: Session = Depends(get_db)):
    q = db.query(MonthlyFixedExpense)
    if month: q = q.filter(MonthlyFixedExpense.month == month)
    return q.order_by(MonthlyFixedExpense.month.desc(), MonthlyFixedExpense.branch_id).all()

@app.post("/monthly-fixed-expenses")
def upsert_monthly_fixed_expense(body: MonthlyFixedExpenseInput, db: Session = Depends(get_db)):
    item = db.query(MonthlyFixedExpense).filter(MonthlyFixedExpense.branch_id == body.branch_id, MonthlyFixedExpense.month == body.month).first()
    if not item:
        item = MonthlyFixedExpense(branch_id=body.branch_id, month=body.month); db.add(item)
    item.amount = body.amount; item.updated_at = datetime.utcnow(); create_audit(db, "UPSERT", "MonthlyFixedExpense", f"branch_id={body.branch_id}; month={body.month}"); db.commit(); db.refresh(item); return item

@app.get("/pending-daily-entries")
def get_pending_entries(db: Session = Depends(get_db)):
    branches = {b.id: b.name for b in db.query(Branch).all()}
    return [{"id": p.id, "branch_id": str(p.branch_id), "branch": branches.get(p.branch_id, "-"), "business_date": str(p.business_date), "revenue": float(p.revenue), "notes": p.notes or "", "expenses": p.expenses or [], "submitted_by": p.submitted_by or "-", "submitted_at": p.submitted_at.isoformat() if p.submitted_at else "", "status": p.status} for p in db.query(PendingDailyEntry).filter(PendingDailyEntry.status == "Pending Approval").order_by(PendingDailyEntry.id.desc()).all()]

@app.post("/pending-daily-entries")
def create_pending_entry(body: PendingEntryInput, db: Session = Depends(get_db)):
    p = PendingDailyEntry(branch_id=body.branch_id, business_date=body.business_date, revenue=body.revenue, notes=body.notes, expenses=[x.dict() for x in body.expenses], submitted_by=body.submitted_by)
    db.add(p); create_audit(db, "CREATE", "PendingDailyEntry", f"branch_id={body.branch_id}; date={body.business_date}"); db.commit(); db.refresh(p); return {"id": p.id, "status": p.status}

@app.post("/pending-daily-entries/{entry_id}/approve")
def approve_pending_entry(entry_id: int, db: Session = Depends(get_db)):
    p = db.query(PendingDailyEntry).filter(PendingDailyEntry.id == entry_id).first()
    if not p: raise HTTPException(status_code=404, detail="Pending entry not found")
    save_daily_entry(DailyEntryInput(branch_id=p.branch_id, business_date=p.business_date, revenue=float(p.revenue), notes=p.notes, expenses=[DailyEntryExpense(**x) for x in (p.expenses or [])]), db)
    p.status = "Approved"; create_audit(db, "APPROVE", "PendingDailyEntry", f"id={entry_id}"); db.commit(); return {"status": "approved"}

@app.post("/pending-daily-entries/{entry_id}/reject")
def reject_pending_entry(entry_id: int, db: Session = Depends(get_db)):
    p = db.query(PendingDailyEntry).filter(PendingDailyEntry.id == entry_id).first()
    if not p: raise HTTPException(status_code=404, detail="Pending entry not found")
    p.status = "Rejected"; create_audit(db, "REJECT", "PendingDailyEntry", f"id={entry_id}"); db.commit(); return {"status": "rejected"}
...
def report_summary(date_from: date, date_to: date, branch_id: Optional[int] = None, include_expenses: Optional[bool] = None, report_mode: str = "full", db: Session = Depends(get_db)):
    return report_data(db, date_from, date_to, branch_id, normalize_report_mode(report_mode, include_expenses))
...
def export_excel(date_from: date, date_to: date, branch_id: Optional[int] = None, include_expenses: Optional[bool] = None, report_mode: str = "full", db: Session = Depends(get_db)):
    mode = normalize_report_mode(report_mode, include_expenses); data = report_data(db, date_from, date_to, branch_id, mode)
    wb = Workbook(); ws = wb.active; ws.title = "Financial Report"; gold, black, white = "D4AF37", "111111", "FFFFFF"
    ws.merge_cells("A1:D1"); ws["A1"] = "Royal Thai Touch ERP - Financial Report"; ws["A1"].font = Font(bold=True, size=16, color=gold); ws["A1"].alignment = Alignment(horizontal="center"); ws["A1"].fill = PatternFill("solid", fgColor=black)
    ws.merge_cells("A2:D2"); ws["A2"] = f"Branch: {data['branch_name']} | Period: {date_from} to {date_to} | Mode: {report_mode_label(mode)}"; ws["A2"].alignment = Alignment(horizontal="center")
    headers = report_columns(mode); ws.append([]); ws.append(headers)
    for cell in ws[4]: cell.font = Font(bold=True, color=white); cell.fill = PatternFill("solid", fgColor=black); cell.border = Border(bottom=Side(style="thin", color="999999"))
    for row in data["daily_rows"]:
        values = [row["date"], row["revenue"]]
        if mode == "full": values.append(row["expenses"])
        if mode != "revenue_only": values.append(row["net_profit"])
        ws.append(values)
    last_row = ws.max_row + 2; ws[f"A{last_row}"] = "TOTAL"; ws[f"B{last_row}"] = data["total_revenue"]
    if mode == "full": ws[f"C{last_row}"] = data["total_expenses"]; ws[f"D{last_row}"] = data["net_profit"]
    elif mode == "revenue_profit": ws[f"C{last_row}"] = data["net_profit"]
    for cell in ws[last_row]: cell.font = Font(bold=True); cell.fill = PatternFill("solid", fgColor="F4E4A6")
    for col in ["A", "B", "C", "D"]: ws.column_dimensions[col].width = 26
...
def export_pdf(date_from: date, date_to: date, branch_id: Optional[int] = None, include_expenses: Optional[bool] = None, report_mode: str = "full", db: Session = Depends(get_db)):
    ...
    mode = normalize_report_mode(report_mode, include_expenses); data = report_data(db, date_from, date_to, branch_id, mode)
    output = io.BytesIO(); doc = SimpleDocTemplate(output, pagesize=A4, rightMargin=28, leftMargin=28, topMargin=28, bottomMargin=28); styles = getSampleStyleSheet()
    ...
    if mode == "full": story.append(Paragraph(f"Total Expenses: IQD {data['total_expenses']:,.0f}", styles["Normal"]))
    if mode != "revenue_only": story.append(Paragraph(f"Net Profit: IQD {data['net_profit']:,.0f}", styles["Normal"]))
    story.append(Spacer(1, 14)); table_data = [report_columns(mode)]
    for row in data["daily_rows"]:
        values = [row["date"], f"IQD {row['revenue']:,.0f}"]
        if mode == "full": values.append(f"IQD {row['expenses']:,.0f}")
        if mode != "revenue_only": values.append(f"IQD {row['net_profit']:,.0f}")
        table_data.append(values)
    table = Table(table_data, repeatRows=1); table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#111111")), ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#D4AF37")), ("GRID", (0, 0), (-1, -1), 0.5, colors.grey), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("ALIGN", (1, 1), (-1, -1), "RIGHT"), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.HexColor("#F7F7F7")])]))
    story.append(table); story.append(Spacer(1, 16)); story.append(Paragraph("Confidential - Royal Thai Touch", styles["Italic"])); doc.build(story); output.seek(0)
