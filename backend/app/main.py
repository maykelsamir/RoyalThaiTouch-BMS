...
app = FastAPI(title="Royal Thai Touch ERP", version="0.7.0")
...
@app.get("/")
def root():
    return {"application": "Royal Thai Touch ERP", "status": "running", "version": "0.7.0"}


@app.get("/health")
def health():
    return {"status": "healthy", "app": "Royal Thai Touch ERP", "version": "0.7.0"}
...
def dashboard(business_date: Optional[date] = Query(None), db: Session = Depends(get_db)):
    ...
    month_revenue = db.query(func.coalesce(func.sum(DailyRevenue.amount), 0)).filter(
        DailyRevenue.business_date >= month_start,
        DailyRevenue.business_date <= target_date,
    ).scalar()
    month_expenses = db.query(func.coalesce(func.sum(Expense.amount), 0)).filter(
        Expense.business_date >= month_start,
        Expense.business_date <= target_date,
    ).scalar()
...
def get_daily_entry(branch_id: int, business_date: date, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "revenue": float(revenue.amount) if revenue else 0,
        "notes": revenue.notes if revenue else "",
        "closed": bool(revenue.closed) if revenue else False,
        "expenses": [{"id": e.id, "category": e.category, "amount": float(e.amount), "notes": e.notes or ""} for e in expenses],
    }
...
def report_summary(date_from: date, date_to: date, db: Session = Depends(get_db)):
    ...
    return {"date_from": str(date_from), "date_to": str(date_to), "total_revenue": total_revenue, "total_expenses": total_expenses, "net_profit": total_revenue - total_expenses, "daily_rows": daily_rows, "branch_totals": list(branch_totals.values())}
