...
def admin_update_daily_entry(body: AdminMonthEntryUpdate, db: Session = Depends(get_db)):
    admin = db.query(AppUser).filter(AppUser.username == body.username, AppUser.active == True).first()
    ...
    if not revenue:
        revenue = DailyRevenue(branch_id=body.branch_id, business_date=body.business_date, amount=body.revenue, notes=body.notes, closed=body.closed)
        db.add(revenue)
    else:
    ...
    details = (f"admin={admin.username}; branch={branch.name}; date={body.business_date}; " f"revenue={old_amount}->{float(body.revenue)}; " f"status={'Closed' if old_closed else 'Open'}->{'Closed' if body.closed else 'Open'}; " f"notes_changed={old_notes != new_notes}; reason={reason}")
    create_audit(db, "ADMIN_UPDATE_DAILY_ENTRY", "DailyEntry", details)
    db.commit()
    db.refresh(revenue)
    return {"status": "updated", "branch_id": revenue.branch_id, "business_date": str(revenue.business_date), "revenue": float(revenue.amount or 0), "closed": bool(revenue.closed), "notes": revenue.notes or ""}
